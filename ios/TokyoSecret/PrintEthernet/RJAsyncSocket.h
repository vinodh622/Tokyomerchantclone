...
@class RJAsyncReadPacket;
@class RJAsyncWritePacket;

extern NSString *const RJAsyncSocketException;
extern NSString *const RJAsyncSocketErrorDomain;

#if !TARGET_OS_IPHONE
extern NSString *const RJAsyncSocketSSLCipherSuites;
extern NSString *const RJAsyncSocketSSLDiffieHellmanParameters;
#endif

enum RJAsyncSocketError
{
  RJAsyncSocketNoError = 0,           // Never used
  RJAsyncSocketBadConfigError,        // Invalid configuration
  RJAsyncSocketBadParamError,         // Invalid parameter was passed
  RJAsyncSocketConnectTimeoutError,   // A connect operation timed out
  RJAsyncSocketReadTimeoutError,      // A read operation timed out
  RJAsyncSocketWriteTimeoutError,     // A write operation timed out
  RJAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
  RJAsyncSocketClosedError,           // The remote peer closed the connection
  RJAsyncSocketOtherError,            // Description provided in userInfo
};
typedef enum RJAsyncSocketError RJAsyncSocketError;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface RJAsyncSocket : NSObject
{
  uint32_t flags;
  uint16_t config;
  ...
  RJAsyncReadPacket *currentRead;
  RJAsyncWritePacket *currentWrite;
  ...
  NSString *setQrCodeTxt;
  NSString *setTableNumber;
  
}

/**
 * RJAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, RJAsyncSocket will automatically create it's own socket queue.
 ...
 *
 * For accepting incoming connections, this means RJAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 *
 * For outgoing connections, this means RJAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, RJAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, RJAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of RJAsyncSocket will be spawned to handle it,
 ...
 *
 * Please note the recommended way of releasing a RJAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a RJAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that RJAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * Example usage:
 *
 * - (void)socket:(RJAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol RJAsyncSocketDelegate
@optional
...
**/
- (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(RJAsyncSocket *)sock;
...
**/
- (void)socket:(RJAsyncSocket *)sock didAcceptNewSocket:(RJAsyncSocket *)newSocket;
...
**/
- (void)socket:(RJAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(RJAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(RJAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(RJAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(RJAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(RJAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(RJAsyncSocket *)sock;
...
**/
- (void)socketDidDisconnect:(RJAsyncSocket *)sock withError:(NSError *)err;
...
**/
- (void)socketDidSecure:(RJAsyncSocket *)sock;
