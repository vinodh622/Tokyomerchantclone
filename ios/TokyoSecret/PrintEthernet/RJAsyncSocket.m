...
NSString *const RJAsyncSocketException = @"RJAsyncSocketException";
NSString *const RJAsyncSocketErrorDomain = @"RJAsyncSocketErrorDomain";

#if !TARGET_OS_IPHONE
NSString *const RJAsyncSocketSSLCipherSuites = @"RJAsyncSocketSSLCipherSuites";
NSString *const RJAsyncSocketSSLDiffieHellmanParameters = @"RJAsyncSocketSSLDiffieHellmanParameters";
#endif

enum RJAsyncSocketFlags
{
...
@interface RJAsyncSocket (Private)
...
/**
 * The RJAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface RJAsyncReadPacket : NSObject
{
  @public
...
@implementation RJAsyncReadPacket
...
/**
 * The RJAsyncWritePacket encompasses the instructions for any given write.
**/
@interface RJAsyncWritePacket : NSObject
{
  @public
  NSData *buffer;
  NSUInteger bytesDone;
...
@implementation RJAsyncWritePacket
...
/**
 * The RJAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface RJAsyncSpecialPacket : NSObject
{
  @public
...
@implementation RJAsyncSpecialPacket
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
  if((self = [super init]))
  {
    ...
    {
      socketQueue = dispatch_queue_create("RJAsyncSocket", NULL);
    }
...
- (BOOL)doAccept:(int)parentSocketFD
{
  ...
  if (delegateQueue)
  {
    ...
    dispatch_async(delegateQueue, ^{
      ...
      // Create RJAsyncSocket instance for accepted socket
      
      RJAsyncSocket *acceptedSocket = [[RJAsyncSocket alloc] initWithDelegate:delegate
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr withQrCode:(NSString*)getQrcodeTxt withTableNo:(NSString*)getTableNo;
{
  setQrCodeTxt = getQrcodeTxt;
  setTableNumber = getTableNo;
  
  NSUserDefaults *printerDefaults = [NSUserDefaults standardUserDefaults];
  [printerDefaults setValue:getQrcodeTxt forKey:@"setQrCodeTxt"];
  [printerDefaults setValue:getTableNo forKey:@"setTableNumber"];
  [printerDefaults synchronize];

  NSLog(@"setQrCodeTxt %@",setQrCodeTxt);
    ...
    else
    {
      ...
      dispatch_async(dispatch_get_main_queue(), ^(void){
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Sorry"
                                                           message:@"Printer not conected. Try again"
                                                           delegate:nil
    ...
    if (i == 0)
  {
    ...
    dispatch_async(delegateQueue, ^{
      ...
      //[theDelegate socket:self didConnectToHost:host port:port];
            [self socket:self didConnectToHost:host port:port];
...
- (void)socket:(RJAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port
{
  
    NSUserDefaults *printerDefaults = [NSUserDefaults standardUserDefaults];

    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"https://chart.googleapis.com/chart?cht=qr&chl=%@&choe=UTF-8&chs=380x380",[printerDefaults objectForKey:@"setQrCodeTxt"]]];
    NSData *data = [NSData dataWithContentsOfURL:url];
    UIImage* generatedQRcodeImg = [[UIImage alloc] initWithData:data];
    
    // To Print Image
    NSData *print_data = [IGThermalSupport imageToThermalData:[self grayscaleImage:generatedQRcodeImg]];
    [sock writeData:print_data withTimeout:-1 tag:0];
    
   // To print Table Number
    NSString* str = [NSString stringWithFormat:@"TABLE : %@",[printerDefaults objectForKey:@"setTableNumber"]];
    NSData* textData = [str dataUsingEncoding:NSUTF8StringEncoding];
    [sock writeData:textData withTimeout:-1 tag:0];
  
    // To feed few empty Lines
...
- (NSError *)badConfigError:(NSString *)errMsg
{
  ...
  return [NSError errorWithDomain:RJAsyncSocketErrorDomain code:RJAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
  ...
  return [NSError errorWithDomain:RJAsyncSocketErrorDomain code:RJAsyncSocketBadParamError userInfo:userInfo];
}
...
- (NSError *)connectTimeoutError
{
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RJAsyncSocketConnectTimeoutError",
                                                       @"RJAsyncSocket", [NSBundle mainBundle],
                                                       @"Attempt to connect to host timed out", nil);
  
  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
  
  return [NSError errorWithDomain:RJAsyncSocketErrorDomain code:RJAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RJAsyncSocketReadMaxedOutError",
                             @"RJAsyncSocket", [NSBundle mainBundle],
                             @"Read operation reached set maximum length", nil);
  
  NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
  
  return [NSError errorWithDomain:RJAsyncSocketErrorDomain code:RJAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RJAsyncSocketReadTimeoutError",
                                                       @"RJAsyncSocket", [NSBundle mainBundle],
                                                       @"Read operation timed out", nil);
  
  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
  
  return [NSError errorWithDomain:RJAsyncSocketErrorDomain code:RJAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RJAsyncSocketWriteTimeoutError",
                                                       @"RJAsyncSocket", [NSBundle mainBundle],
                                                       @"Write operation timed out", nil);
  
  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
  
  return [NSError errorWithDomain:RJAsyncSocketErrorDomain code:RJAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
  NSString *errMsg = NSLocalizedStringWithDefaultValue(@"RJAsyncSocketClosedError",
                                                       @"RJAsyncSocket", [NSBundle mainBundle],
                                                       @"Socket closed by remote peer", nil);
  
  NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
  
  return [NSError errorWithDomain:RJAsyncSocketErrorDomain code:RJAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
  ...
  return [NSError errorWithDomain:RJAsyncSocketErrorDomain code:RJAsyncSocketOtherError userInfo:userInfo];
}
                        ...
                        tag:(long)tag
{
  ...
  RJAsyncReadPacket *packet = [[RJAsyncReadPacket alloc] initWithData:buffer
                                                            startOffset:offset
                                                              maxLength:length
                                                                timeout:timeout
                                                             readLength:0
...
- (void)maybeDequeueRead
{
  ...
  if ((currentRead == nil) && (flags & kConnected))
  {
    if ([readQueue count] > 0)
    {
      ...
      if ([currentRead isKindOfClass:[RJAsyncSpecialPacket class]])
      {
        LogVerbose(@"Dequeued RJAsyncSpecialPacket");
      ...
      {
        LogVerbose(@"Dequeued RJAsyncReadPacket");
...
- (void)doReadData
{
  ...
  else if (totalBytesReadForCurrentRead > 0)
  {
    ...
    {
      id theDelegate = delegate;
      RJAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
  ...
  RJAsyncWritePacket *packet = [[RJAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
  ...
  if ((currentWrite == nil) && (flags & kConnected))
  {
    if ([writeQueue count] > 0)
    {
      ...
      if ([currentWrite isKindOfClass:[RJAsyncSpecialPacket class]])
      {
        LogVerbose(@"Dequeued RJAsyncSpecialPacket");
      ...
      {
        LogVerbose(@"Dequeued RJAsyncWritePacket");
...
- (void)doWriteData
{
  ...
  // Note: This method is not called if theCurrentWrite is an RJAsyncSpecialPacket (startTLS packet)
  ...
  else
  {
    ...
    if (bytesWritten > 0)
    {
      ...
      {
        id theDelegate = delegate;
        RJAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
  ...
  RJAsyncSpecialPacket *packet = [[RJAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
{
  RJAsyncSocket *asyncSocket = (RJAsyncSocket *)connection;
...
- (void)maybeStartTLS
{
  ...
  if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
  {
    ...
    RJAsyncSpecialPacket *tlsPacket = (RJAsyncSpecialPacket *)currentRead;
    ...
    // 7. kCFStreamSSLLevel
    // 8. RJAsyncSocketSSLCipherSuites
    // 9. RJAsyncSocketSSLDiffieHellmanParameters
    ...
    value = [tlsSettings objectForKey:RJAsyncSocketSSLCipherSuites];
    if (value)
    {
    ...
    value = [tlsSettings objectForKey:RJAsyncSocketSSLDiffieHellmanParameters];
    if (value)
    {
...
- (void)maybeStartTLS
{
  ...
  if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
  {
    ...
    NSAssert([currentRead isKindOfClass:[RJAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
    NSAssert([currentWrite isKindOfClass:[RJAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)addStreamListener:(RJAsyncSocket *)asyncSocket
{
  LogTrace();
...
+ (void)removeStreamListener:(RJAsyncSocket *)asyncSocket
{
  LogTrace();
  NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
    RJAsyncSocket *asyncSocket = [(RJAsyncSocket *)pInfo retain];
